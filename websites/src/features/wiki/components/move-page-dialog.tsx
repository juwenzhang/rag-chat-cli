...
import type { WikiOut, WikiPageDetailOut } from "@/lib/api/shared/types";
...
export function MovePageDialog({
  ...
  useEffect(() => {
    if (!open) return;
    const id = window.setTimeout(() => setTargetId(currentWikiId), 0);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      const moved = await wikiPageService.movePage(pageId, {
