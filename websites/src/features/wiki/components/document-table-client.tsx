...
import type { DocumentOut } from "@/lib/api/shared/types";
...
export function DocumentTableClient({ documents: initial }: Props) {
  ...
  const onDelete = async () => {
    ...
    try {
      await documentService.deleteDocument(pendingDelete.id);
      ...
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {docs.length} document{docs.length === 1 ? "" : "s"} — personal notes not
            linked to any wiki
          </p>
        </div>
