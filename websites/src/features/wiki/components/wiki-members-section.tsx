...
import type { WikiMemberOut, WikiRole } from "@/lib/api/shared/types";
...
export function WikiMembersSection({
  ...
  const refresh = async () => {
    try {
      onMembersChanged(await wikiService.listMembers(wikiId));
    } catch {
  ...
  const onInvite = async (e: React.FormEvent) => {
    ...
    setInviting(true);
    try {
      await wikiService.addMember(wikiId, {
  ...
  const changeRole = async (userId: string, role: WikiRole) => {
    try {
      await wikiService.updateMemberRole(wikiId, userId, { role });
      await refresh();
    } catch (err) {
