...
import type { WikiPageShareOut } from "@/lib/api/shared/types";
...
export function WikiPageShareDialog({
  ...
  const createOrFetch = useCallback(async () => {
    setLoading(true);
    try {
      const result = await wikiPageService.createPageShare(pageId);
      setShare(result);
    } catch {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => void createOrFetch(), 0);
  ...
  const onRevoke = async () => {
    ...
    setRevoking(true);
    try {
      await wikiPageService.revokePageShare(share.token);
