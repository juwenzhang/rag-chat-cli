...
export function DocumentEditorClient({ document: initial }: Props) {
  ...
  const setDocumentTitleOverride = useWikiStore(
    (state) => state.setDocumentTitleOverride
  );
  ...
  const flush = async () => {
    ...
    setStatus("saving");
    try {
      await documentService.updateDocument(docRef.current.id, {
  ...
  const onTitleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    scheduleSave();
    setDocumentTitleOverride(docRef.current.id, next);
  };
  ...
  const onDelete = async () => {
    try {
      await documentService.deleteDocument(docRef.current.id);
    } catch {
