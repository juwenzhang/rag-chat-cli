...
from tui.ui import ChatView, PromptSession
from tui.ui.console import make_console
...
async def _build_memory() -> _MemoryBundle:
    ...
    from service.memory.chat_memory import DbChatMemory, FileChatMemory
    ...
    try:
        from service.db.session import current_session_factory, init_engine
...
async def _resolve_local_user_id() -> uuid.UUID | None:
    ...
    """
    from service.auth.errors import TokenExpiredError, TokenInvalidError
    from service.auth.tokens import decode_token
    from tui import auth_local
...
async def run_legacy_chat(
    ...
    from tui.ui.prompt import SlashDispatcher
    ...
    def _kb_label(kb: Any) -> str:
        ...
        from service.knowledge import FileKnowledgeBase, PgvectorKnowledgeBase
    ...
    def _kb_root_hint(kb: Any) -> str:
        ...
        from service.knowledge import FileKnowledgeBase
    ...
    async def _kb_show(args: list[str]) -> None:
        ...
        if info.chunk_count > len(chunks):
            view.system_notice(f"… {info.chunk_count - len(chunks)} more chunk(s) hidden")
    ...
    async def _kb_summary() -> None:
        ...
        if not docs:
            view.system_notice(f"{label} kb empty · {_kb_root_hint(kb)}")
            view.system_notice("tip: /save after a good Q+A to start filling it, or `main ingest`")
            return
    ...
    async def _kb_sync(_args: list[str]) -> None:
        ...
        """
        from service.knowledge import FileKnowledgeBase, PgvectorKnowledgeBase
        from service.knowledge.local import DEFAULT_KB_ROOT
        ...
        for d in local_docs:
            ...
            try:
                ...
                continue
            view.system_notice(
                f"  ✓ {info.title[:50]} → {new_info.id[:8]} · {new_info.chunk_count} chunk(s)"
            )
            ok += 1
        view.system_notice(
            f"sync done · {ok}/{len(local_docs)} OK" + (f" · {failed} failed" if failed else "")
        )
    ...
    async def _save(args: list[str]) -> None:
        ...
        content = f"Q: {last_turn['user']}\n\nA: {last_turn['assistant']}"
        try:
    ...
    def _reflect(args: list[str]) -> None:
        if not args:
            ...
            view.system_notice(f"reflect {'on' if on else 'off'} · threshold {thr:.2f}")
            return
        first = args[0].lower()
        if first in {"on", "off"}:
            new = first == "on"
    ...
    async def _maybe_auto_save(user_text: str, asst_text: str) -> None:
        ...
        if not user_text.strip() or not asst_text.strip():
            return
        from service.knowledge import ReflectionCritic
    ...
    async def _register(_args: list[str]) -> None:
        from service.auth.errors import AuthError, EmailAlreadyExistsError
