...
from tui.chat_app import run_legacy_chat
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ingest.add_argument("--title", default=None, help="document title (default: filename stem)")
    ingest.add_argument(
        "--chunk-size",
        type=int,
        default=None,
...
def _iter_target_files(root: Path, *, recursive: bool) -> list[Path]:
    ...
    return sorted(p for p in glob if p.is_file() and p.suffix.lower() in TEXT_EXTENSIONS)
...
async def _run_ingest_async(ns: argparse.Namespace) -> int:
    ...
    from service.knowledge import DocumentIngestor
    ...
    if getattr(ns, "run_async", False):
        from service.workers import JobSpec, RedisJobQueue
    ...
    print(
        ...
        f"{total_chunks} chunks written" + (f", {failed} failed" if failed else "")
    )
...
def _run_worker(ns: argparse.Namespace) -> int:
    ...
    from service.knowledge import DocumentIngestor
    from service.llm.ollama import OllamaClient
    from service.workers import JobSpec, RedisJobQueue, Worker
    from settings import settings
    ...
    async def _handle_ingest(job: JobSpec) -> None:
        ...
        try:
            ...
            print(
                f"[worker] ingested {path} → doc={result.document_id} chunks={result.chunk_count}"
            )
        finally:
            await llm.aclose()

    async def _run() -> None:
        from service.workers.queue import DEFAULT_QUEUE_NAME
