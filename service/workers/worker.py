...
Wiring (typical CLI use, see :mod:`app.cli`)::
    ...
    from service.workers import RedisJobQueue, Worker
...
if TYPE_CHECKING:
    from service.workers.queue import JobSpec, RedisJobQueue
...
"""Async callable that processes one :class:`~service.workers.queue.JobSpec`.
...
class Worker:
    ...
    async def run_forever(self) -> None:
        ...
        logger.info(
            ...
            self._queue.name,
            ", ".join(sorted(self._handlers)) or "<none>",
        )
        try:
            while self._running:
                try:
