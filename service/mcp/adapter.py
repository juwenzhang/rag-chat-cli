...
satisfy :class:`service.tools.Tool` — they can then be ``ToolRegistry.register``'ed
right next to local :class:`~service.tools.FunctionTool` s. The ReAct loop in
:class:`~service.chat.service.ChatService` doesn't know or care which side of
...
from service.tools import ToolError, ToolResult

if TYPE_CHECKING:
    from service.mcp.client import MCPStdioClient, MCPToolInfo

__all__ = ["McpTool", "wrap_as_tools"]


class McpTool:
    """One remote MCP tool, exposed as a local :class:`service.tools.Tool`.
    ...
    async def call(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        """
        from service.mcp.client import MCPError
