...
from service.providers import decrypt_api_key
...
async def build_llm_for_user(
    ...
    ``await client.aclose()`` when done. :class:`service.chat.service.ChatService`
    ...
    # ORM modules into ``service.chat.service`` unconditionally.
    from service.llm.ollama import OllamaClient
    from service.llm.openai import OpenAIClient
...
async def _pick_provider(session: AsyncSession, *, user_id: uuid.UUID) -> Provider | None:
