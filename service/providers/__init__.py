...
async def list_providers(session: AsyncSession, *, user_id: uuid.UUID) -> list[ProviderInfo]:
    ...
    *,
    ...
    if is_default:
        ...
        prior = await session.scalars(
            select(Provider).where(Provider.user_id == user_id, Provider.is_default.is_(True))
        )
        for p in prior:
            p.is_default = False
...
async def update_provider(
    ...
    *,
    ...
    """
    row = await get_provider(session, user_id=user_id, provider_id=provider_id)
    ...
    if clear_api_key:
        if api_key is not None:
            raise ProviderValidationError("pass either api_key or clear_api_key, not both")
...
async def test_connectivity(*, type: str, base_url: str, api_key: str | None) -> dict[str, Any]:
...
async def list_models(*, type: str, base_url: str, api_key: str | None) -> list[dict[str, Any]]:
    ...
    async with httpx.AsyncClient(timeout=_HTTP_TIMEOUT) as client:
        if type == "ollama":
            r = await client.get(base_url.rstrip("/") + "/api/tags", headers=headers)
            r.raise_for_status()
            payload = r.json()
            return [
                {
        ...
        # openai-compatible
        r = await client.get(base_url.rstrip("/") + "/v1/models", headers=headers)
        r.raise_for_status()
        payload = r.json()
        return [
            {
...
async def delete_ollama_model(*, base_url: str, api_key: str | None, model: str) -> None:
...
async def show_ollama_model(*, base_url: str, api_key: str | None, model: str) -> dict[str, Any]:
...
async def running_ollama_models(*, base_url: str, api_key: str | None) -> list[dict[str, Any]]:
    ...
    *,
    ...
    if row is None:
        if description is None or description == "":
            return
        row = ModelMetadata(provider_id=provider_id, model=model, description=description)
        session.add(row)
    else:
...
async def seed_default_ollama_for_user(
    ...
    * the ``OLLAMA_BASE_URL`` setting is unset.
    ...
    existing = await session.scalar(select(Provider).where(Provider.user_id == user_id).limit(1))
    ...
    row = Provider(
        ...
        base_url=base_url.rstrip("/"),
        api_key_encrypted=(
            encrypt_api_key(settings.ollama.api_key) if settings.ollama.api_key else None
        ),
        is_default=True,
        enabled=True,
    )
    ...
    await session.commit()
    await session.refresh(row)
    logger.info("seeded default Ollama provider for user %s -> %s", user_id, base_url)
    return row
