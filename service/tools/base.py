...
This is the boundary between the LLM's intent (a :class:`service.llm.client.ToolCall`)
and the host-side side-effects that fulfill it. The split is intentional:

* :mod:`service.llm.client` owns the LLM-facing shapes (``ToolSpec``, ``ToolCall``).
* :mod:`service.tools` owns the execution-facing shapes (``Tool``, ``ToolResult``,
  ``ToolError``, ``ToolRegistry``).

Concrete tools live next to feature code (or in :mod:`service.tools.builtin` once
that exists). The :class:`ToolRegistry` in :mod:`service.tools.registry` is what
the ReAct loop in :mod:`service.chat.service` will consult.
"""
...
from service.llm.client import ToolSpec
...
class Tool(Protocol):
    ...
    async def call(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        :class:`service.llm.client.ToolCall`. Implementations should
