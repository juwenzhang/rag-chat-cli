...
* **SQLite (lightweight local smoke checks)**: pgvector has no SQLite
  dialect. We swap the type to :class:`_JSONVectorFallback` (a
  :class:`TypeDecorator` around ``JSON``) which serialises ``list[float]``
  as JSON text. Round-trips work; similarity queries do not.
