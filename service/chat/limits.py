...
* A tool that hangs indefinitely (``tool_timeout_s`` wraps
  :meth:`service.tools.Tool.call` in :func:`asyncio.wait_for`).
...
construction parameter on :class:`~service.chat.service.ChatService`.
"""
...
#: :class:`~service.chat.service.ChatService` when no overrides are given.
