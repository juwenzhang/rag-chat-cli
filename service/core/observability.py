...
1. :func:`get_tracer` — returns either a real ``opentelemetry.trace.Tracer``
   ...
   the seams in :class:`service.chat.service.ChatService` so a production
...
class Tracer(Protocol):
    """Minimum tracer surface used by :class:`service.chat.service.ChatService`."""
    ...
    def record_usage(
        ...
        * OpenAI → ``prompt_tokens`` / ``completion_tokens``
        ...
        if model is not None and model in self._prices_per_1k:
            ...
            added_cost = (prompt_tokens / 1000.0) * in_rate + (
    ...
    def record_usage_dict(
        ...
        *,
        ...
        if usage is None:
            ...
            return
        prompt = usage.get("prompt_tokens") or usage.get("prompt_eval_count") or 0
        completion = usage.get("completion_tokens") or usage.get("eval_count") or 0
        self.record_usage(
            model=model,
