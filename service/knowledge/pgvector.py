...
Implements :class:`service.knowledge.base.KnowledgeBase` against the ``chunks``
...
from service.knowledge.base import DocumentInfo, KnowledgeHit
from service.knowledge.rerank import NoopReranker, Reranker
from service.llm.client import LLMClient, LLMError
...
class PgvectorKnowledgeBase:
    ...
    async def search(self, query: str, *, top_k: int = 4) -> list[KnowledgeHit]:
        ...
        :class:`service.llm.client.LLMError` — callers (``ChatService``) catch
    ...
    async def add_document(
        ...
        *,
        ...
        Wraps :class:`~service.knowledge.ingest.DocumentIngestor` so the same
